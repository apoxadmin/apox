...
function getGoodEvents($user)
{
	...
	$sql = 'SELECT DISTINCT event.event_id, event_name, UNIX_TIMESTAMP(event_date) AS date'
        ...
        . ' AND event.event_id = shift.event_id AND shift.shift_id = signup.shift_id'
...
if(!isset($event))
{
	...
	?>
	<form name="selecttracking" method="GET" action="/tracking/service/edit.php">
...
else
{
	...
	<?php forms_hiddenInput('setdetails',"/tracking/service/edit.php?event=$event");

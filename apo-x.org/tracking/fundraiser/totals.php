...
{
	$date = NOW; // now
	 $sql = 'SELECT count( * ) AS events, sum( hours ) AS h, sum( chairs ) AS c'
        . ' FROM event NATURAL JOIN tracking'
        . " WHERE eventtype_id = '9' AND event_ic = 0 AND user_id = '$user' "
...
function show_eventsTrack($name, $user)
{
	...
	<tr id="r<?php echo $user ?>">
		<?php 
			$total = getTotal($user);
			echo "<td><a href=\"/tracking/fundraiser/user.php?user=$user\">$name</a></td>";

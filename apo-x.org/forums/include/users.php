...
function phorum_user_check_session( $cookie = PHORUM_SESSION )
{
	...
	{
		$user = phorum_user_get( $_SESSION['id'] );
		if ( $user["active"] ) {
			$GLOBALS["PHORUM"]["user"] = $user;
			return true;
		}
...
{
	// XXX
	die('phorum_user_create_session');
...
{
	// XXX
	die('phorum_user_check_username');

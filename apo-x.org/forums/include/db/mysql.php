...
function phorum_db_get_group_members($group_id, $status = PHORUM_USER_GROUP_REMOVE)
{
    ...
    // XXX joining with the real table that can sort
    ...
    $sql="select {$PHORUM['user_group_xref_table']}.user_id, {$PHORUM['user_group_xref_table']}.status from user, {$PHORUM['user_group_xref_table']} where user.user_id = {$PHORUM['user_group_xref_table']}.user_id and group_id in ($group_id)";
...
{
	// XXX
	// change function to get from chi user table when possible
	...
	// get all ids that are in the phorum table
	$sql = "select user.user_id AS user_id "
		. " from {$PHORUM['user_table']} NATURAL JOIN user where user.user_id in ($user_ids)";
	$res = mysql_query($sql, $conn);
	if ($err = mysql_error()) phorum_db_mysql_error("$err: $sql");
	
	$remove = array();
	
	// leave behind the ids not in the phorum table
	while($rec=mysql_fetch_assoc($res))
        $remove[] = $rec["user_id"];

	$user_id = array_diff($user_id, $remove);

	// create record in phorum table for each user id
	foreach($user_id as $user)
		phorum_db_user_add_internal($user);

    $users = array();

    // XXX
    $sql = "select user.user_id AS user_id, user_name AS username, user_email as email, "
		. " hide_email, active, user_data, signature, threaded_list, posts, admin, threaded_read, "
...
function phorum_db_user_get_list(){
   ...
   $users = array();
   $sql = "select user_id, user_name AS username from user order by username asc";
   $res = mysql_query($sql, $conn);
   if ($err = mysql_error()) phorum_db_mysql_error("$err: $sql");
...
function phorum_db_user_check_field($field, $value, $operator="=", $return_array=false){
    ...
    $sql = 'select user.user_id '
		. " from {$PHORUM['user_table']} NATURAL JOIN user where "
		. implode(" and ", $clauses);
...
/** XXX
 * This function executes a query to add the phorum part of the user
 * data to go with the chi part
 */

function phorum_db_user_add_internal($user){
    $PHORUM = $GLOBALS["PHORUM"];

    $conn = phorum_db_mysql_connect();

    $sql = "insert into {$PHORUM['user_table']} (user_id, active) VALUES ('$user', '1')";
...
function phorum_db_search_users($search)
{
	...
	// XXX
    $sql = 'select user_id, user_name AS username, user_email as email '
		. "from user where user_name like '%$search%' or user_email like '%$search%'order by username";
...
{
	// XXX
	// die('users shouldnt need to wait for approval');
...
function phorum_db_get_subscribed_users($forum_id, $thread, $type){
	...
	// XXX
    $sql = "select DISTINCT(b.user_email) from {$PHORUM['subscribers_table']} as a, user as b where a.forum_id=$forum_id and (a.thread=$thread or a.thread=0) and a.sub_type=$type and b.user_id=a.user_id $userignore";

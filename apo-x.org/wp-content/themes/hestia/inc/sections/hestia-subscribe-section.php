	...
	function hestia_subscribe( $is_shortcode = false ) {
		...
		if ( $is_shortcode === false && (bool) $hide_section === true ) {
			if ( is_customize_preview() ) {
				echo '<section class="subscribe-line subscribe-line-image" id="subscribe" data-sorder="hestia_subscribe" style="display: none"></section>';
			}
			return;
		}
		...
		hestia_before_subscribe_section_trigger();
			?>
		<section class="subscribe-line subscribe-line-image <?php echo esc_attr( $wrapper_class ); ?>" id="subscribe" data-sorder="hestia_subscribe" style="background-image: url('<?php echo get_theme_mod( 'hestia_subscribe_background', get_template_directory_uri() . '/assets/img/about.jpg' ); ?>');">
			<?php
			if ( is_customize_preview() ) {
			?>

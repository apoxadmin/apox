...
include_once dirname(dirname(__FILE__)) . '/include/mobilesql.php';
...
<form style="margin-top:20px;" action="/mobile/mine.php" method="GET">
...
function show_meetingTracking($events, $user)
{
	...
	?>
		<tr>
			...
			<td class="<?php echo $class ?>">
				<?php forms_checkbox_disabled("p[{$event['event_id']}]", 1, $event['p']>0) ; ?>
			</td>
		</tr>
	<?php } ?>
	<tr>
...
function show_commTracking($user)
{
	...
	<table cellspacing="1" style="width: 75%">
		...
		<tr>
			<th style="width: 20px">Comm Credit?</th>
			<th>Details</th>
			<th>Dues?</th>
		</tr>
		<tr>
...
function show_pictureTracking($user)
{
	...
	$term_id = $result['class_id'];
	echo "<div class=\"general\" style=\"text-align:center;\"><h4>{$name['name']}'s <br> Tracking for {$term_nick} Term</h3></div>";

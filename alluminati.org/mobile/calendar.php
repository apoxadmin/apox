		...
		for(var column=0; column<7; column++)
		{
			...
			for(var i=0; i<days[row][column].length; i++)
			{
				...
				if(show)
				{
					...
					if(txt != "1")
						inner += "<p class=\"" + myevent.style + "\"><a " + signup + " href=\"/mobile/show.php?id=" + 
							myevent.id + "\">" + myevent.title + "</a></p>";
					else
						inner += "<p class=\"" + myevent.fourc + "\"><a " + signup + " href=\"/mobile/show.php?id=" + 
...
show_calendarheader($head, 'fill()');

?>
<br>
<br>
<span style="font-size:20px;">&nbsp;<?php echo $my?></span>
<div style="float:right; padding:0px 10px 10px 0px;">
	<a href="calendar.php?month=<?php echo $m1 ?>&amp;year=<?php echo $y1 ?>">
	<img style="padding:0px 10px 0px 0px; height:50px; width:50px;" src="/images/mobile/arrowleft.jpg" />
	</a><a href="calendar.php?month=<?php echo $m2 ?>&amp;year=<?php echo $y2 ?>">
	<img style="padding:0px 2px 0px 10px; height:50px; width:50px;" src="/images/mobile/arrowright.jpg" /></a>
	
</div>
<form>
<div style="float:left;padding-top:6px; padding-left:6px;">
<font  class="big"> Filter: </font>
<select style="font-size: 10px" name="filters" onchange="fill()" style="margin: 0px; padding: 0px">
	<option value="0" <?php echo ($filter==0)?'selected':'' ?>>All Events</option>
	...
	<option value="5" <?php echo ($filter==5)?'selected':'' ?>>Meetings</option>
...
<tr >
	<td class="heading">Sun</td>
	<td class="heading">Mon</td>
	<td class="heading">Tue</td>
	<td class="heading">Wed</td>
	<td class="heading">Thu</td>
	<td class="heading">Fri</td>
...
for($i=0,$k=0; $i<$weeks; $i++) // One row for each week
{
	...
	for($j=0;$j<7;$j++) // One column for each day of the week
	{
		...
		if(date("F",$today12AM)==date("F",strtotime('now')) && date("d",$today12AM)==date("d",strtotime('now')) && date("Y",$today12AM)==date("Y",strtotime('now')))
			$style = "calToday";
		...
		if($_SESSION['class'] == 'admin') // make a link to add a new event if admin
		{
			...
			echo "<a href=\"/mobile/edit.php?page=create&date=$date\">";
